...
- **Behind a reverse proxy the derived callback URL is now correct**: the entrypoint starts uvicorn with `--proxy-headers --forwarded-allow-ips ${FORWARDED_ALLOW_IPS:-*}`, so `X-Forwarded-Proto`/`-Host` are honoured (uvicorn's default trusts only `127.0.0.1`, which never matches a proxy on a Docker network). `OIDC_REDIRECT_URL` is still respected and still the belt-and-braces option when the proxy does not set those headers
- **The callback is served at both `/api/auth/oidc/callback` and `/api/auth/oidc/callback/`.** Providers store one literal redirect URI and a trailing slash is an ordinary way to write it (Authentik's `strict:` entries especially). Starlette would normally 307 `/path/` to `/path`, but the SPA catch-all in `main.py` matches every path and wins the match first — so the callback returned `index.html` with HTTP 200, stranding the authorization code with no cookie set and no error shown. Both spellings are routed to the same handler rather than redirected, because a 307 rebuilds the URL from the request scheme and would put the code on the wire in cleartext
- **`spa_fallback` answers a JSON 404 for an unmatched `/api/...` path** instead of the SPA shell. Serving HTML with HTTP 200 for a missing endpoint is what made the above silent, and it hands the frontend an HTML body everywhere it expects JSON. It also refuses a path that resolves outside `STATIC_DIR` — nothing upstream normalises `..`, so `/../../config/potation.key` was readable
...
- `services/potation/drm.py` — **the four-way delivery branch**, decided once from the DRM type and the *shape* of the key material, so nothing downstream guesses. `aaxc` (Adrm + 16-byte key and iv), `aax` (Adrm + 4-byte activation bytes), `unencrypted` (`Mpeg`, or no DRM), `unsupported` (everything else, with a reason). `ffmpeg_decrypt_args()` turns a plan into input flags; an unencrypted plan contributes **none**, because passing `-audible_key` for a file that has no encryption is an ffmpeg error rather than a harmless extra flag
  - **The failure this exists to prevent is a quiet one.** A 4-byte activation blob passed where a 16-byte AAXC key belongs does not error — ffmpeg writes a file. A corrupt `.m4b` that reaches Chaptarr and then Audiobookshelf is discovered by whoever tries to listen to it. So key material is length-checked (and rejected as non-hex or odd-length) before any plan is usable
  - **One deliberate divergence from Libation**: it treats anything that is not Widevine or Adrm as unencrypted mp3 delivery; we refuse an *unrecognised* DRM type instead. A classified `drm_unsupported` error and an honestly un-downloaded book beat a corrupt file
  - Pure — no network, database or subprocess — which is what lets all four arms be tested, three of them being unreachable against a real account on demand
  - A rejection reason never quotes the key material: reasons are shown to people and written to logs
- `services/potation/license.py` also owns **reuse**: `stored_license()` returns a persisted licence only while its CDN link is still good (`URL_EXPIRY_MARGIN`, 5 min — an expired link 403s *after* the download has started), and `license_for_download()` prefers it over buying another. A `Download` licence counts against Audible's daily allowance, so this is the difference between a retry being free and being rationed. `request_license(decrypt_voucher=True)` recovers the AAXC key/iv via `audible.aescipher.decrypt_voucher_from_licenserequest` and persists them **Fernet-encrypted** under `potation.key`; it is off by default so the census stays a measurement. `_persist` only *writes* key columns when the incoming licence carries them — a census re-probe of the same title must not blank key material a download already stored
- `services/potation/download.py` — the transfer, and **the stage the current engine cannot do anything with**: LibationBridge fires `ProcessAsync` into a detached `Task` with no handle, so there is nothing to cancel and no progress beyond one conflated percentage. Cancellation is threaded through from the start here rather than retrofitted
  - The whole file lands on disk before ffmpeg sees it. AAXC is MP4, ffmpeg needs the `moov` atom and seeks, and Audible's CDN files are not reliably faststart — Libation has a `MoveMoovToBeginning` option for the same reason. That is also what makes resume possible
  - `should_cancel` and `on_progress` are polled on a clock (`POLL_INTERVAL`), not per chunk: both usually touch the database, and a 500 MB book at 64 KiB a chunk would call them eight thousand times
  - Written to `<name>.part`, renamed only on success, so a partial download can never be mistaken for a finished one. A cancel **keeps** the part file and resumes with an HTTP `Range` request; `discard_partial()` is the explicit throw-away
  - **A CDN that ignores `Range` and resends the whole body restarts from zero.** Appending would produce a file of plausible size and corrupt content — the worst outcome available here
  - `_total_bytes()` reads `Content-Range`, because on a resumed request `Content-Length` is only what *remains*; the naive reading makes a half-finished download report 100%
  - Disk space is checked before starting, at `SPACE_FACTOR` (2.2×) — decrypting writes a second copy alongside the download
- `services/potation/decrypt.py` — one ffmpeg invocation does the decryption, remux, chapters, tags and cover. `-c copy` throughout: faster, lossless, and no shifted chapter timeline. Libation's `AllowLibationFixup` transforms are deliberately not reproduced — Chaptarr and Audiobookshelf own presentation
  - **The last chapter is extended to the end of the file.** Audible's `chapter_info` describes the *content*; the delivered file is longer by a brand outro, and Libation pads the final chapter for the same reason. Inside one `.m4b` that is cosmetic — but **split-by-chapter is a toggle in Settings**, and there the tail is the end of the book silently absent from the last file. `parse_chapters()` never *shortens* a chapter, which would invent a truncation rather than fix one
  - **`write_identity_tags()` exists because ffmpeg cannot do it.** The mp4 muxer maps a fixed set of metadata keys onto atoms and drops the rest without a warning, so `ASIN=` in the ffmetadata document produces a file with no ASIN at all — caught by the check written for exactly that. The tag is written afterwards with `mutagen`, as `----:com.apple.iTunes:ASIN`, which is the shape `reconcile._tag_asin` reads; a round-trip check pins the two together. A failure to write it **fails the job**, because the file would be unattributable the moment `import_mode=move` runs
  - Decryption flags must precede `-i` — they configure the demuxer reading that input. After it, ffmpeg accepts them silently and reads the file undecrypted
  - Cancellation means killing a child process, so `should_cancel` is polled while ffmpeg runs and terminates it (`terminate`, then `kill` after 5s). Written to `<dest>.partial` and renamed only after tagging succeeds
  - `safe_filename()` emits `<Title> [<ASIN>].m4b`: Chaptarr's `DownloadedBooksScan` fallback and Audiobookshelf's `getASIN()` both read the bracketed ASIN out of the path, so it is the second identity carrier alongside the tag
- `services/potation/quota.py` — the daily-download ledger. **Not Audible's cap**: that is enforced upstream and surfaces as a refused licence whatever this table says. This is (a) reporting, (b) a self-imposed guard someone can set *below* the real ceiling, and (c) history that predates the limit
  - **Every success is recorded, limit configured or not** — Libation's own reasoning. Recording only while a limit is set would hand out a free extra allowance the day someone sets one
  - **The window is a rolling 24 hours, deliberately.** Audible calls its cap daily without saying in which timezone; a rolling window never permits more than `limit` in *any* 24-hour span, so it cannot err in the direction that gets an account throttled. Entries age out by the clock — there is nothing to run
  - **No limit by default.** Guessing Audible's number and enforcing the guess costs real downloads for no benefit. A malformed, zero or negative `system_settings` value also reads as unlimited: a bad setting blocking every download is a worse failure than not enforcing a guard that Audible already enforces underneath
  - `is_audible_plus` is recorded but never branched on — whether subscription titles share the allowance is a question to answer from data, not to guess at now
  - Usage is per account (`account_id=None` gives the unscoped total, for display); `resets_at` is when the oldest entry in the window ages out
- `services/potation/queue.py` — the persistent job queue, and **where the blocked features come from**. Cancel, pause, per-book status during a bulk run, classified retry and progress that survives a restart are all one missing thing today: a job row. LibationBridge holds progress in a `ConcurrentDictionary` and fires work into a detached `Task`; here the `download_jobs` row is the truth
  - **The pipeline is injected.** `run_job(pipeline=...)` defaults to the real licence → download → decrypt chain, but the seam is what lets claiming, retry, cancellation, resume and concurrency be tested exhaustively without an Audible account
  - **Claiming is a conditional `UPDATE ... WHERE state='queued'`**, checked by row count. One uvicorn worker means a race should not arise — but "should not" is how a book gets downloaded twice, and the conditional form is free
  - **`cancel_requested()` reads past SQLAlchemy's identity map.** The hazard is *not* transaction isolation (a raw `SELECT` sees the committed value on both backends) — it is that an ORM re-query hands back the instance already loaded in this session. It reads via raw SQL **and** rolls back, which expires the map so the whole session agrees; either alone suffices today, together it survives the obvious tidy-up of rewriting the read in ORM terms
  - **`reset_orphans()` requeues, it does not fail.** A job interrupted by a crash resumes from its `.part` file; failing it would discard a mostly-finished transfer and spend the daily allowance again
  - Retry is classified: only `RETRYABLE_ERRORS` (`network`, `unknown`) come back, up to `max_retries`. A licence denial or unsupported DRM fails at once rather than spending quota to fail three times. A cancellation is `cancelled`, never `error`, so it is not retried
  - `run_job` never raises for an ordinary failure — a failed job is a row with an `error_code`, not an exception escaping into the worker and taking the queue with it. `enqueue_many` goes through `reconcile.check_bulk_enqueue`, checked against the *new* work so re-submitting an already-queued selection is not refused for its size
...
- `scripts/potation-census.py` — `login` / `sync` / `census` / `download` / `disconnect` subcommands to measure DRM exposure against a real account. **`disconnect` is not optional housekeeping**: `login` registers a device on the Amazon account, Amazon caps how many one may hold, and deleting the scratch directory does not release it — the registration lives at Amazon, so removing the local credentials only makes the slot unrecoverable. Samples 25 titles by default; `--sample 0` sweeps everything and prompts first. Keeps its database, `potation.key` and any downloaded audio under a gitignored `.potation-census/` because both hold live Audible credentials
  - **`download` is Phase B's gate**, and the only thing that exercises the `aaxc` arm — a licence being *offered* and a file being *decryptable* are separate claims. It drives the production path (`enqueue` → `claim_next` → `run_job` with the default pipeline) rather than a parallel copy, then checks the artifact: it parses, its duration matches the library, chapters are present and reach the end, `reconcile._tag_asin` reads the ASIN back, the filename carries it, and **the audio decodes**
  - **The decode pass is the only check that catches a wrong key or a truncated transfer.** With `-c copy` nothing decodes during the remux, so a wrong 16-byte key yields a file ffprobe describes quite happily; and the duration ffprobe reports comes from the `moov` atom, which survives a missing tail. It decodes the whole file by default for that reason
  - **An unencrypted delivery does not clear the gate.** The picked title's stored licence decides which of `drm.py`'s arms actually ran, and anything but `aaxc` reports "the pipeline worked, but this is not a pass" — the same overclaim the census was built to avoid. For the same reason the default pick is the shortest title over `REAL_BOOK_MINUTES` (60), not the shortest overall: a sample is small, likely `Mpeg`, and would sail through every check while proving nothing
...
- Tests: `scripts/test-queue.py` — the queue's own decisions with the pipeline injected: dedupe, priority order, atomic claiming, retry classification, a cancel written on another session (raw read **and** ORM view), orphan recovery, and that one failing job does not stall the rest. CI job `queue` gates `merge`
- Tests: `scripts/test-quota.py` — the ledger: recording without a limit configured, the rolling window ageing out by the clock, per-account scoping, `resets_at`, and a malformed limit reading as unlimited. CI job `quota` gates `merge`
- Tests: `scripts/test-decrypt.py` — real ffmpeg, real audio, read back with `mutagen` and `ffprobe`: chapters into the file, the ASIN round-tripping through `reconcile._tag_asin`, cover art, cancellation leaving nothing, and a classified ffmpeg failure. The `aaxc`/`aax` arms end to end are **not** covered (they need a real encrypted file and licence) and the suite says so. CI job `decrypt` gates `merge`
- Tests: `scripts/test-download.py` — the transfer, against a real in-process HTTP server: cancellation keeping the partial file, resume, a CDN that ignores `Range`, unknown `Content-Length`, and the disk-space check. CI job `download` gates `merge`
- Tests: `scripts/test-drm.py` — the delivery branch, key-shape rejection (nine ways), encryption at rest, and licence reuse. CI job `drm` gates `merge`
...
This is a single-maintainer repository and there is no human review gate; the CI suites (`chaptarr`, `liberate`, `reconcile`, `firstrun`, `backup`, `drm`, `download`, `decrypt`, `quota`, `queue`, `potation`, `oidc`, `smoke`) are the gate. Adding a check to one of them is how you make a new invariant enforceable.
