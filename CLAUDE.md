...
This is a single-maintainer repository and there is no human review gate; the CI suites (`chaptarr`, `liberate`, `reconcile`, `firstrun`, `backup`, `drm`, `download`, `decrypt`, `quota`, `potation`, `oidc`, `smoke`) are the gate. Adding a check to one of them is how you make a new invariant enforceable.
